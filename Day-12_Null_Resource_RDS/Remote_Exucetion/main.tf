...
resource "aws_instance" "sql_runner" {
    ...
    key_name               = "C:/Users/anura/.ssh/id_ed25519.pub"                # Replace with your key pair name
...
resource "null_resource" "remote_sql_exec" {
    ...
    connection {
        type        = "ssh"
        user        = "ec2-user"
        private_key = file("C:/Users/anura/.ssh/id_ed25519")   # Replace with your PEM file path
...
# ADD RDS creation script only accessible internally si disable public access 
# Remote provisioner server also should create in same vpc 
# enable secrets fro secret manager and call secrets into RDS for this process vpc endpoint is require or nat gateway is required to access secrets to rds internal as secret manger is not in side VPC service 